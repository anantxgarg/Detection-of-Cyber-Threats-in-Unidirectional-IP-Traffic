...
class DetectionEngine:
    def __init__(self, redis_host='localhost'):
        ...
        self.baseline = BaselineEngine(host=redis_host)
    ...
    def run(self):
        print("Detection Engine started. Listening to events:live...")
        last_id = '$'
        
        while True:
            try:
                streams = self.r.xread({'events:live': last_id}, count=100, block=1000)
                ...
                for stream_name, messages in streams:
                    for msg_id, msg_data in messages:
                        ...
                        try:
                            # Reconstruct event mapping from flat dict string types
                            event_data = dict(msg_data)
                            event_data['data'] = json.loads(msg_data['data'])
                            event_data['ts'] = float(msg_data['ts'])
                            event_data['src_port'] = int(msg_data['src_port'])
                            event_data['dst_port'] = int(msg_data['dst_port'])
                            ...
                            for detector in self.detectors:
                                ...
                                if result:
                                    print(f"🔥 ALERT [{result.threat_class.value}] Confidence: {result.confidence} Target: {result.src_ip}")
                                    self.r.xadd('alerts:live', {
                                        'alert': result.model_dump_json()
                                    })
...
if __name__ == "__main__":
    ...
    engine = DetectionEngine(redis_host=args.redis_host)
    engine.run()
