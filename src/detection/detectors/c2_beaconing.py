...
class C2BeaconingDetector:
    ...
    def analyze(self, event: NormalizedEvent) -> DetectionResult | None:
        ...
        # Track flow timestamps per source-destination pair
        ts_list_key = f"c2_ts:{src_ip}:{dst_ip}"

        pipe = self.r.pipeline()
        pipe.lpush(ts_list_key, event.ts)
        pipe.ltrim(ts_list_key, 0, 49)
        pipe.expire(ts_list_key, 3600)
        ...
        # Redis list order is not guaranteed to represent chronological order.
        # Sort timestamps before calculating inter-arrival intervals.
        ts_array = np.sort(
            np.array([float(t) for t in raw_ts], dtype=float)
        )
        ...
        if cov < self.cov_threshold:
            ...
            if len(pxx) > 0 and np.max(pxx) > 5 * np.mean(pxx):
                is_periodic = True
        ...
        if is_periodic:
            ...
            if not self.r.get(alert_key):
                self.r.setex(alert_key, 600, 1)

                # Keep confidence safely bounded between 0.4 and 0.9.
                confidence = max(
                    0.4,
                    min(0.9, 0.9 - (cov * 0.5))
                )
